      ...
      <div className="min-w-0 space-y-4">
        <div
          ...
          className={`relative rounded-2xl border-2 border-dashed p-10 text-center transition-colors sm:p-14 ${
            dragActive
              ? 'border-[#C04ACD] bg-[#fae8ff]/30'
              : 'border-[#e5e7eb] bg-white hover:border-[#C04ACD]/40 hover:bg-[#fae8ff]/10'
          }`}
        >
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-[#C04ACD]/15 to-[#7E2D8E]/15 text-[#7E2D8E]">
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#F2F4F7] bg-white px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          <FileText className="h-4 w-4 flex-shrink-0 text-[#7E2D8E]" />
...
function SubmittingView({ count }: { count: number }) {
  return (
    <div className="rounded-2xl border border-[#F2F4F7] bg-white p-10 text-center">
      <Loader2 className="mx-auto h-8 w-8 animate-spin text-[#C04ACD]" />
          ...
          <tbody className="font-mono text-xs">
            {columns.map((col) => (
              <tr key={col.name} className="border-b border-[#f1f5f9]">
                <td className="whitespace-nowrap px-3 py-2 text-[#7E2D8E]">
                  {col.name}
                </td>
                <td
